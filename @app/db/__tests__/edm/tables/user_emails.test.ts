...
async function addEmail(client: PoolClient, email: string) {
  const {
    ...
    `insert into edm.user_emails (email) values ($1) returning *`,
    [email]
  );
  return row;
}
...
it("cannot manually create a verified email", () =>
  withUserDb(async client => {
    const promise = client.query(
      `insert into edm.user_emails (email, is_verified) values ($1, true) returning *`,
...
it("cannot manually mark an email as verified", () =>
  withUserDb(async client => {
    const email = await addEmail(client, "newemail@example.com");
    const promise = client.query(
      "update edm.user_emails set is_verified = true where id = $1",
      [email.id]
    );
    await expect(promise).rejects.toThrow(
      /permission denied for (table|relation) user_emails/
...
it("can promote a verified email to primary over another verified email", () =>
  withUserDb(async client => {
    ...
    const {
      rows: [email2],
    } = await client.query("select * from edm.make_email_primary($1)", [
      email.id,
    ]);
...
it("cannot promote a non-verified email to primary", () =>
  withUserDb(async client => {
    ...
    expect(email.is_primary).toBe(false);
    const promise = client.query(
      "select * from edm.make_email_primary($1)",
      [email.id]
    );
