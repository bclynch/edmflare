...
export async function login(
  ...
  const {
    rows: [row],
  } = await client.query(`select * from edm_private.login($1, $2)`, [
    username,
    password,
  ]);
  return row;
...
it("too many login attempts resets after 5 minutes", () =>
  withRootDb(async client => {
    ...
    const {
      ...
      "select * from edm_private.user_secrets where user_id = $1",
      [testUser.id]
    );
    ...
    await client.query(
      "update edm_private.user_secrets set first_failed_password_attempt = now() - interval '5 minutes' where user_id = $1",
      [testUser.id]
    );
