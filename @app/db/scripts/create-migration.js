...
async function main() {
  ...
  if (name && !name.match(NAME_REGEX)) {
    console.error(
      'Migration name is if the wrong format. We recommend SCHEMA.TYPE[.SUBJECT][.SUBJECT].ACTION; e.g. edm.schema.create or edm.column.users.bio.add_length_check'
    );
    process.exit(2);
  }
  if (!name || !name.length) {
    console.error(
      "Migration name is required, e.g. 'yarn db migrate:create edm.users.create'"
    );
    process.exit(1);
  }
